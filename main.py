...
from utils import get_name, get_position, send_email_smtp
...
def reload_config():
    ...
    bot.auth_channels = [
        str(bot.config[server].get("verify_channel", "")) for server in bot.config
    ]
...
async def verification_message(message: discord.Message):
    ...
    verification_code = (
        code.group() if (code := CODE_RE.search(message_content)) else None
    )
    ...
    if email_address:  # User sent an email address
        ...
        if (
            ...
            big_code = (
                f"https://dsu.gael.in/verify.php?user={author.id}&code={big_code}"
            )
            try:
                send_email_smtp(email_address, str(author), code, big_code)
        ...
        if (
            bot.cursor.execute(
                ...
                (author.id, int(verification_code)),
            )
            > 0
        ):
    ...
    if message_react:
        await message.add_reaction(message_react)
    await message.delete(delay=25)
    if message_response:
        await (await message.reply(message_response)).delete(delay=25)
...
async def confirm_roles(member: discord.Member):
    bot.cursor.execute(
        "SELECT email,name,position FROM discord.users WHERE id = %s", member.id
    )
    ...
    if user_info[1]:
        try:
            await member.edit(
                nick=user_info[1], reason=f"Verified to {user_info[1]} ({user_info[0]})"
            )
        ...
        if user_info[2] == "professor":
            await member.add_roles(
                member.guild.get_role(
                    int(bot.config[str(member.guild.id)][f"instructor_role"])
                ),
                reason=f"Verified to {user_info[1]} ({user_info[0]})",
            )
            await member.add_roles(
                member.guild.get_role(
                    int(bot.config[str(member.guild.id)][f"student_role"])
                ),
                reason=f"Verified to {user_info[1]} ({user_info[0]})",
            )
